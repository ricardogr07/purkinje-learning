...
from purkinje_learning import BO_ecg, OptimParam
from purkinje_learning.bo_utils import Prior
...
def test_mse_jaxbo_creates_objective_function():
    ...
    var_params = [OptimParam("cv", np.array([2.0]), np.array([4.0]), Prior.UNIFORM)]
...
def test_bo_loop_one_iteration():
    ...
    bo_loop_gp_patch = patch("purkinje_learning.bo_ecg.GP", return_value=mock_gp)
