...
from purkinje_learning import BO_PurkinjeTree, BO_PurkinjeTreeConfig, MeshSuffix
...
class MockFractalTree:
    def __init__(self, params=None):
        self.params = params or MockParameters()
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_initialize_calls_fractal_tree_correctly(config):
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_build_fractal_tree_parameters_sets_expected_values(config):
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_apply_modifications_to_tree_modifies_expected_params(config):
    # Setup
    tree = BO_PurkinjeTree("patient01", [0, 1, 2, 3], config, MagicMock())
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_apply_modifications_to_tree_both_valid(config):
    tree = BO_PurkinjeTree("patient01", [0, 1, 2, 3], config, MagicMock())
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_apply_modifications_to_tree_lv_only(config):
    tree = BO_PurkinjeTree("patient01", [0, 1, 2, 3], config, MagicMock())
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_apply_modifications_to_tree_both_scalar_raises(config):
    tree = BO_PurkinjeTree("patient01", [0, 1, 2, 3], config, MagicMock())
...
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_initialize_coupling_points_returns_expected_shapes(config):
    # Mock PMJ indices
...
@patch("purkinje_learning.bo_purkinje_tree.onp.savetxt")
@patch("purkinje_learning.bo_purkinje_tree.FractalTree", new=MockFractalTree)
@patch("purkinje_learning.bo_purkinje_tree.FractalTreeParameters", new=MockParameters)
def test_initialize_coupling_points_with_modify_and_save(mock_savetxt, config):
    # Mock PMJ indices
    lv_pmj = onp.array([1, 2, 3])
    rv_pmj = onp.array([4, 5])
...
@patch("purkinje_learning.bo_purkinje_tree.PurkinjeTree")
def test_run_ECG_executes_and_returns_expected_outputs(mock_purkinje_tree):
    ...
    bo.kmax = 3
    bo.LVfractaltree = MockFractalTree()
    bo.RVfractaltree = MockFractalTree()
    bo._apply_modifications_to_tree = MagicMock()
    bo._initialize_coupling_points = MagicMock()
    bo._activate_purkinje_and_myo = MagicMock()
    bo._validate_fractal_tree = MagicMock(side_effect=lambda tree, _: tree)
