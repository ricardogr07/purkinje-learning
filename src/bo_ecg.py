...
class BO_ecg:
    ...
    def extract_overlapping_section(self, ground_truth, predicted, delay):
        ...
        ), "the function should extract the overlapping sections"
    ...
    def plot_ecg_match(self, predicted, filename_match=None):
        ...
        for ax, lead_name in zip(axs.ravel(), self.ground_truth.dtype.names):
            ...
            # ax.plot(ground_truth[lead_name],'b', alpha=0.6, label="Ground truth")
            # t_pred = t_min_loss - len2 + 1 + onp.arange(len2)
            # ax.plot(t_pred, predicted[lead_name],'r', alpha=0.6, label="BO")
            ...
            ax.plot(predicted[lead_name], "tab:red", alpha=0.6, label="BO")
        ...
        if cross_correlation:
            ...
            for t_shift in np.arange(len1 - 1, len2 - cut_fin):
                ...
                for label in ground_truth.dtype.names:
                    pred, gt = self.extract_overlapping_section(
                        predicted[label], ground_truth[label], t_shift
                    )
            ...
            for label in ground_truth.dtype.names:
                ...
                mse = mean_squared_error(
                    ground_truth[label][:trim], predicted[label][:trim]
                )
                errors.append(mse)
    ...
    def mse_jaxbo(self, ground_truth, variable_parameters):
        ...
        assert (
            len(set(dist_types)) == 1
        ), "The prior distribution for all variable parameters must be equal"
        ...
        def f(x):
            ...
            var_params = self.set_dictionary_variables(
                var_parameters=variable_parameters, x_values=x
            )

            # obtain predicted ecg
            try:
                predicted, LVtree, RVtree = self.bo_purkinje_tree.run_ECG(
                    n_sim=0, modify=True, side="both", **var_params
                )
    ...
    def set_initial_training_data(self, N, noise=0.0):
        self.noise = noise
    ...
    def set_test_data(self):
        ...
        if self.dim == 1:
            ...
            X_star = self.lb_params + (self.ub_params - self.lb_params) * lhs(
                self.dim, nn
            )
    ...
    def bo_loop(self, X, y, X_star, true_x, options, save_info=False):
        ...
        for it in range(options["nIter"]):
            ...
            opt_params = gp_model.train(norm_batch, rng_key, num_restarts=5)  # 100
            ...
            if save_info:
                ...
                mean_it, std_it = gp_model.predict(
                    X_star, **kwargs
                )  # of normalized data
                ...
                def acq_fun(x):
                    return gp_model.acquisition(x, **kwargs)
            ...
            # Training error
            train_error = np.mean(
                (gp_model.predict(X, **kwargs)[0] - norm_batch["y"]) ** 2
            )
    ...
    def plot_mse(self, X, y, N, file_name):
        ...
        best_y_it = (
            " (in iteration " + str(idx_best - N + 1) + ")"
            if idx_best >= N
            else " (in training points)"
        )
        textstr = (
            params
            + "\nbest_x = "
    ...
    def update_purkinje_tree(self, X, y, var_parameters):
        ...
        best_var_parameters = self.set_dictionary_variables(
            var_parameters=var_parameters, x_values=best_x
        )

        ecg_bo, LVtree_bo, RVtree_bo = self.bo_purkinje_tree.run_ECG(
            modify=True, side="both", **best_var_parameters
        )
